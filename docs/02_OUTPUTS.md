...
**Where to inspect intermediate state.** Each loader writes its output as a stand-alone CSV in `data/interim/` so Allen and Darren can eyeball their loader output in Excel without running the full pipeline:

- `data/interim/prices.csv` — raw long-format daily prices (output of `load_prices`)
- `data/interim/market_features.csv` — derived rolling features (output of `features.build_market_features`)
- `data/interim/fundamentals.csv` — Phase 1 synthetic; Phase 2 real SEC (output of `load_fundamentals`)
- `data/interim/macros.csv` — Phase 1 synthetic; Phase 2 real FRED (output of `load_macros`)
- `data/interim/labels.csv` — Label A (`label_a`, `forward_max_drawdown`) plus a `label_b` column that stays NaN until Darren's 8-K loader ships

Each interim file has a one-line provenance header (`# generated by ews.pipeline on <iso>, loader=<name>`); read back via `pd.read_csv(path, comment='#')`.
